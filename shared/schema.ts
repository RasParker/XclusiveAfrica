...
export const comments = pgTable("comments", {
  ...
  parent_id: integer("parent_id"), // for replies - self-reference added later
...
export const subscriptions = pgTable("subscriptions", {
  ...
  previous_tier_id: integer("previous_tier_id").references(() => subscription_tiers.id), // For tracking tier changes
...
export const notifications = pgTable("notifications", {
  ...
  actor_id: integer("actor_id").references(() => users.id), // ID of user who triggered the notification
...
export const proration_credits = pgTable("proration_credits", {
  ...
  applied_to_payment_id: integer("applied_to_payment_id").references(() => payment_transactions.id), // which payment this credit was applied to
...
// Audit log table for tracking admin actions and system events
export const audit_logs = pgTable("audit_logs", {
  ...
  resource_type: text("resource_type").notNull(), // user, post, subscription, setting, etc.
  resource_id: text("resource_id"), // ID of the affected resource
  details: jsonb("details"), // JSON object with change details
...
// System alerts table for monitoring and alerting
export const system_alerts = pgTable("system_alerts", {
  id: serial("id").primaryKey(),
  type: text("type").notNull(), // performance, security, error, maintenance
  severity: text("severity").notNull().default("medium"), // low, medium, high, critical
  title: text("title").notNull(),
  message: text("message").notNull(),
  source: text("source").notNull(), // api, database, authentication, etc.
...
export const systemAlertsRelations = relations(system_alerts, ({ one }) => ({
...
export const insertSystemAlertSchema = createInsertSchema(system_alerts).pick({
  type: true,
  severity: true,
  title: true,
  message: true,
