...
export const CreatorSettings: React.FC = () => {
  ...
  // Initialize currentEmail from user context
  useEffect(() => {
    if (user?.email) {
  ...
  const handleProfilePhotoUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      try {
        ...
        const response = await fetch('/api/cloudinary/profile-photo', {
          method: 'POST',
          body: formData,
        });
  ...
  const handleCoverPhotoUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      try {
        ...
        // Upload to backend using Cloudinary
        const response = await fetch('/api/cloudinary/cover-photo', {
          method: 'POST',
          body: formData,
        });
                  ...
                  {isCategoryLoading ? (
                    ...
                    <>
                      ...
                      <div className="space-y-4">
                        <div className="flex items-center justify-between">
                          ...
                          <Badge variant="outline" className="text-xs">
                            {creatorCategories.length} of 4 selected
                            {creatorCategories.length < 4 && ` • ${4 - creatorCategories.length} more available`}
                          </Badge>
                        </div>
                            ...
                            {creatorCategories.map((creatorCat: any) => {
                              ...
                              return (
                                <div
                                  key={creatorCat.id}
                                  className={`
                                    relative p-3 rounded-lg border-2 transition-all duration-200 shadow-md
                                    ${isPrimary 
                      ...
                      <div className="space-y-4 pt-4 border-t border-border/50">
                        ...
                        {creatorCategories.length >= 4 && (
                          <div className="p-3 rounded-lg bg-muted/50 border border-dashed border-border/50">
                            <div className="text-sm text-muted-foreground text-center">
                              You've reached the maximum of 4 categories. Remove a category to add a different one.
                            </div>
                          </div>
                        )}
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          {allCategories
                            ...
                            .map((category) => {
                              const IconComponent = categoryIcons[category.icon] || User;
                              const isAtLimit = creatorCategories.length >= 4;
                              ...
                              return (
                                <button
                                  ...
                                  onClick={async () => {
                                    if (isAtLimit) {
                                      toast({
                                        ...
                                        description: "You can select a maximum of 4 categories (1 primary + 3 secondary).",
                                        variant: "destructive",
                                      });
                                      return;
                                    }
                                  ...
                                  className={`
                                    p-3 rounded-lg border border-dashed transition-all duration-200 text-left group
                                    ${isAtLimit 
                                  ...
                                  <div className="flex items-center space-x-3">
                                    <IconComponent 
                                      className={`w-5 h-5 transition-transform ${
                                        isAtLimit ? '' : 'group-hover:scale-110'
                                      }`} 
                                      style={{ color: category.color }}
                                    />
                                    <div>
                      ...
                      <div className="space-y-4 pt-4 border-t border-border/50">
                        ...
                        {creatorCategories.length >= 4 && (
                          <div className="p-3 rounded-lg bg-muted/50 border border-dashed border-border/50">
                            <div className="text-sm text-muted-foreground text-center">
                              Remove a category first to create a custom one.
                            </div>
                          </div>
                        )}
                  ...
                  <CardContent className="space-y-6">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex items-center justify-between p-4 rounded-lg bg-muted/20">
                        <span className="text-sm">{user?.email || currentEmail}</span>
