...
import { Search, Users, Star, Filter, Heart, MessageSquare, Share2, Image, Video, Palette, Dumbbell, Music, Laptop, ChefHat, Shirt, Gamepad2, Briefcase, Home, GraduationCap, UserPlus } from 'lucide-react';
...
export const Explore: React.FC = () => {
  ...
  useEffect(() => {
    const fetchCreators = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const allCreatorsList = [...transformedCreators, ...CREATORS];
          setAllCreators(allCreatorsList);

          // Fetch like counts and follower counts for real creators
          const likeCounts: {[key: string]: number} = {};
          const followerCounts: {[key: string]: number} = {};
          for (const creator of transformedCreators) {
            const actualCreatorId = creator.id.replace('real_', '');
            try {
              const response = await fetch(`/api/creators/${actualCreatorId}/likes-count`);
              if (response.ok) {
                const data = await response.json();
                likeCounts[creator.id] = data.count || 0;
              }
            } catch (error) {
              console.error(`Error fetching like count for creator ${actualCreatorId}:`, error);
              likeCounts[creator.id] = 0;
            }

            try {
              const followerResponse = await fetch(`/api/creators/${actualCreatorId}/followers-count`);
              if (followerResponse.ok) {
                const followerData = await followerResponse.json();
                followerCounts[creator.id] = followerData.count || 0;
              }
            } catch (error) {
              console.error(`Error fetching follower count for creator ${actualCreatorId}:`, error);
              followerCounts[creator.id] = 0;
            }
          }
          setCreatorLikeCounts(likeCounts);
          setCreatorFollowerCounts(followerCounts);
    ...
    fetchCreators();
  }, [loadingCategories, categories, user]);
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center gap-4 text-sm text-muted-foreground">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        {creatorFollowerCounts[creator.id] || 0} followers
                      </div>
                      <div className="flex items-center gap-1">
                        <Heart className="w-3 h-3" />
                        {creatorLikeCounts[creator.id] || 0} likes
                      </div>
                    </div>
