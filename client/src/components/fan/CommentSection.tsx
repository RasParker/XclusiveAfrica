...
export const CommentSection: React.FC<CommentSectionProps> = ({
    ...
    try {
      ...
      if (response.ok) {
        ...
        const formattedComments = fetchedComments.map((comment: any) => ({
          ...
          // Properly format nested replies
          replies: (comment.replies || []).map((reply: any) => ({
            id: reply.id.toString(),
            user: {
              id: reply.user?.id.toString() || '1',
    ...
    try {
      ...
      if (response.ok) {
        ...
        setComments(prev => {
          ...
          // Update comment count based on new state
  ...
  const handleAddReply = async (parentId: string, replyContent: string) => {
    ...
    try {
      // Call backend API to persist the reply
      const response = await fetch(`/api/posts/${postId}/comments`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        ...
        body: JSON.stringify({
          ...
          parent_id: parseInt(parentId) // Add parent_id for replies
        })
      });
      ...
      if (response.ok) {
        ...
        const reply: Comment = {
          id: newReplyData.id.toString(),
          user: {
            id: newReplyData.user?.id.toString() || user?.id?.toString() || '1',
            username: newReplyData.user?.username || user?.username || 'current_user',
            avatar: newReplyData.user?.avatar || user?.avatar
          },
          content: newReplyData.content,
          likes: 0,
          liked: false,
          createdAt: newReplyData.created_at,
          replies: []
        };
        ...
        setComments(prev => {
          ...
          const totalCount = newComments.reduce((total, comment) => total + 1 + comment.replies.length, 0);
