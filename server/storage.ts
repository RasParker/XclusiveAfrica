...
export interface IStorage {
  ...
  followCreator(followerId: number, creatorId: number): Promise<boolean>;
  unfollowCreator(followerId: number, creatorId: number): Promise<boolean>;
  isFollowingCreator(followerId: number, creatorId: number): Promise<boolean>;
  getFollowedCreators(followerId: number): Promise<any[]>;
  getCreatorFollowers(creatorId: number): Promise<any[]>;
  getCreatorFollowerCount(creatorId: number): Promise<number>;

  // Audit logging methods
  createAuditLog(log: InsertAuditLog): Promise<AuditLog>;
  getAuditLogs(limit?: number, userId?: number, action?: string, resourceType?: string): Promise<AuditLog[]>;
  ...
  getSystemAlerts(status?: string, severity?: string, limit?: number): Promise<SystemAlert[]>;
  acknowledgeSystemAlert(alertId: number, userId: number): Promise<SystemAlert | undefined>;
  resolveSystemAlert(alertId: number, userId: number): Promise<SystemAlert | undefined>;
  getActiveSystemAlerts(): Promise<SystemAlert[]>;
  getCriticalSystemAlerts(): Promise<SystemAlert[]>;
}
...
export class DatabaseStorage implements IStorage {
  ...
  async getFanFavorites(fanId: number): Promise<any[]> {
    const favorites = await db
      ...
      .leftJoin(categories, eq(users.primary_category_id, categories.id))
  ...
  // Creator follow methods
  async followCreator(followerId: number, creatorId: number): Promise<boolean> {
    try {
  ...
  async isFollowingCreator(followerId: number, creatorId: number): Promise<boolean> {
    try {
  ...
  async getFollowedCreators(followerId: number): Promise<any[]> {
    const followedCreators = await db
      ...
      .from(follows)
      .innerJoin(users, eq(follows.creator_id, users.id))
      .where(eq(follows.follower_id, followerId))
  ...
  async getCreatorFollowers(creatorId: number): Promise<any[]> {
    const followers = await db
      ...
      .where(eq(follows.creator_id, creatorId))
      .orderBy(desc(follows.created_at));

    return followers;
  }

  async getCreatorFollowerCount(creatorId: number): Promise<number> {
    try {
      const [result] = await db
        .select({ count: sql<number>`count(*)` })
        .from(follows)
  ...
  async acknowledgeSystemAlert(alertId: number, userId: number): Promise<SystemAlert | undefined> {
    try {
  ...
  async resolveSystemAlert(alertId: number, userId: number): Promise<SystemAlert | undefined> {
    try {
      const [alert] = await db
        .update(system_alerts)
        .set({
  ...
  async getCriticalSystemAlerts(): Promise<SystemAlert[]> {
    try {
      return await db
        .select()
        .from(system_alerts)
