...
import { users, posts, comments, post_likes, comment_likes, subscriptions, subscription_tiers, reports, creator_likes, creator_favorites, follows, users as usersTable, posts as postsTable, subscriptions as subscriptionsTable, subscription_tiers as tiersTable, comments as commentsTable, conversations as conversationsTable, messages as messagesTable } from '../shared/schema';
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Serve uploaded files statically (legacy support)
  ...
  app.post("/api/auth/login", async (req, res) => {
    try {
      ...
      // Set session data (for cookie-based auth)
      req.session.userId = user.id;
      req.session.user = userWithoutPassword;

      // Set token as cookie (for admin routes)
  ...
  app.get("/api/users/me", async (req, res) => {
    try {
      ...
      // Remove password and return user data in consistent format
      const { password: _, ...userWithoutPassword } = user;
      res.json({ user: userWithoutPassword });
    } catch (error) {
  ...
  // Get personalized feed for a user (follows + subscriptions)
  ...
  app.get('/api/posts/:id', async (req, res) => {
    try {
      ...
      await db.update(posts)
        .set({ 
          views_count: sql`COALESCE(${posts.views_count}, 0) + 1`,
          updated_at: new Date()
        })
  ...
  app.post("/api/posts/:postId/comments", async (req, res) => {
    try {
      ...
      const validatedData = insertCommentSchema.parse({
        ...req.body,
        user_id: userId, // Use authenticated user from session
        post_id: postId
      });
    ...
    try {
      ...
      const recentPosts = await db
        .select({
          ...
          views_count: postsTable.views_count,
...
app.get('/api/fan/:fanId/favorites', async (req, res) => {
  ...
  // File upload routes (redirect to Cloudinary)
  app.post("/api/upload/profile-photo", (req, res) => {
    res.status(301).json({ 
      success: false, 
      message: 'This endpoint has moved. Please use /api/cloudinary/profile-photo instead.',
      redirect_to: '/api/cloudinary/profile-photo'
    });
  });

  app.post("/api/upload/cover-photo", (req, res) => {
    res.status(301).json({ 
      success: false, 
      message: 'This endpoint has moved. Please use /api/cloudinary/cover-photo instead.',
      redirect_to: '/api/cloudinary/cover-photo'
    });
  });

  app.post("/api/upload/post-media", (req, res) => {
    res.status(301).json({ 
      success: false, 
      message: 'This endpoint has moved. Please use /api/cloudinary/post-media instead.',
      redirect_to: '/api/cloudinary/post-media'
    });
  });
...
app.post('/api/conversations', async (req, res) => {
  ...
  app.get("/api/creators/:creatorId/follow/:followerId", async (req, res) => {
    try {
      const creatorId = parseInt(req.params.creatorId);
  ...
  app.get("/api/creators/:creatorId/followers-count", async (req, res) => {
    try {
      const creatorId = parseInt(req.params.creatorId);
      const count = await storage.getCreatorFollowerCount(creatorId);
      res.json({ count });
    } catch (error) {
